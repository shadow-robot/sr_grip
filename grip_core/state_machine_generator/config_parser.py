...
class StateMachineConfigParser(object):
    ...
    """
        Class gathering all required operations to parse a YAML file describing a state machine
    """

    def __init__(self, yaml_file_path, states_directory_path):
        """
            Load the YAML file and initialise class' attributes

            @param yaml_file_path: Path to the YAML file describing the state machine to be built
            @param states_directory_path: Path to the directory containing the states used to generate state machines
        """
        # Read the YAML file
        with open(yaml_file_path) as file_:
            self.state_machine_content = yaml.safe_load(file_)
    ...
    def _parse(self):
        ...
        # Initialise an empty list that contain the description of the potential nested state machine
